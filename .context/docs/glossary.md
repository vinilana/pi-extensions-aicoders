...
- **Judges** — validation extension that compares a SPEC/result pair using prompt judges and optional scripts before accepting completion.
- **pendingSpec** — SPEC generated by judges with only the remaining work after a failed evaluation.
...
- **judges_evaluate** — LLM tool in `extensions/judges/application/register-tool.ts`. Avalia SPEC versus resultado/evidências e retorna passed, pendências e pendingSpec.
...
- **/judges** — slash command in `extensions/judges/application/register-command.ts`. Inspeciona config, define SPEC ativa, executa avaliação manual e reseta estado dos judges
